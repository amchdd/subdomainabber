...
func TestIntigritiLoadsEveryAccessibleProgram(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/external/researcher/v1/programs":
			_, _ = response.Write([]byte(`{"maxCount":1,"records":[{"id":"p1","handle":"acme","name":"Acme","following":true,"minBounty":{"value":50,"currency":"EUR"},"maxBounty":{"value":5000,"currency":"EUR"},"status":{"id":1,"value":"Open"},"type":{"id":2,"value":"Bug bounty"},"webLinks":{"detail":"https://app.intigriti.com/programs/acme"}}]}`))
		case "/external/researcher/v1/programs/p1":
	...
	client := NewIntigriti(server.Client(), server.URL+"/external/researcher", "segredo")
	...
	if !programs[0].Bounty || !programs[0].Assets[0].Bounty {
		t.Fatalf("programa com recompensa não foi identificado: %+v", programs[0])
	}
}
